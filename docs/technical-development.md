...
- ✅ Prompt input for action (settings panel text input).
- ✅ Structured action semantic plan (Claude provider returns ActionSemanticPlan).
- ✅ Generated pose template (per-frame parts transforms).
- ✅ Optional override frames (base64 PNGs for complex frames).
- ✅ Preview against selected character (saved action appears in context menu).
- ✅ Save only after validation passes (saveGeneratedPack flow with pack registry integration).
...
- Action generation UI is in the "AI Action" section of the settings panel (text prompt + Generate button).
- Generated action packs are saved to `userData/packs/` and auto-registered in the `customActions` dynamic catalog.
- Context menu shows custom action packs below the separator when available (fetched via `listActionPacks` IPC).
- `composeAnimation` handles pack actions via `packActionId` in ComposeConfig — falls back to `resolveActionPack()` IPC call to load pose JSON from the action pack file.
- `getAction()` now checks `customActions` Map as fallback after the built-in ACTIONS table.
- Lazy-loading: if an action isn't found in catalog, `playAction` fetches the manifest via IPC and registers it.`
